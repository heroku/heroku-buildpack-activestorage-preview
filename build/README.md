...
**After every change to your formulae, perform the following** from this `build/` subdirectory to rebuild the images for each stack:
    ...
    docker build --pull --tag heroku-activestorage-preview-build-heroku-26:amd64 --platform linux/amd64 --file heroku-26.Dockerfile .
    docker build --pull --tag heroku-activestorage-preview-build-heroku-26:arm64 --platform linux/arm64 --file heroku-26.Dockerfile .
...
You can e.g. `bash` into each of the images you built using their tag:
    ...
    docker run --rm -ti heroku-activestorage-preview-build-heroku-26:amd64 bash
    docker run --rm -ti heroku-activestorage-preview-build-heroku-26:arm64 bash
...
*You need to pass in the necessary `AWS_` and `S3_` env vars using `--env-file` or `--env`.*
    ...
    docker run --rm -ti heroku-activestorage-preview-build-heroku-26:amd64 bob deploy ffmpeg-8.1
    docker run --rm -ti heroku-activestorage-preview-build-heroku-26:arm64 bob deploy ffmpeg-8.1
    docker run --rm -ti heroku-activestorage-preview-build-heroku-24:amd64 bob deploy ffmpeg-7.1.3
    docker run --rm -ti heroku-activestorage-preview-build-heroku-24:arm64 bob deploy ffmpeg-7.1.3
    docker run --rm -ti heroku-activestorage-preview-build-heroku-22 bob deploy ffmpeg-5.1.8
